function [grid_k1]  = find_grid_kmid(par_min_Dk,v_dm,v_de,grid_mask,grid_topo)
%
%%
...
% *********************************************************************** %
%
% NOTE: this finds the layer bottom depth (and hence k value)
%       closest to an input depth
% determine grid size
[jmax imax] = size(grid_topo);
kmax = length(v_dm);
% create top and bottom boundary vectors
v_dbot = [v_de(1) v_dm(1:kmax-1)];
v_dtop = [v_dm(1:kmax-1) 0.0];
% first process topo:
% (i) apply mask
